...
func main() {
	...
	if err := db.Raw("SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'runs' AND column_name = 'run_triggers_fired_at')").Scan(&hasRunTriggersFired).Error; err != nil {
		logger.Warnf("Failed to inspect runs.run_triggers_fired_at column: %v", err)
		hasRunTriggersFired = true // fail safe: never backfill on uncertainty
	}
	...
	// One-time cutover for run triggers: mark every pre-existing applied run as already-fired so the
	// orchestrator's run-trigger worker only queues downstream runs for applies that happen AFTER this
	// upgrade. Without this, adding the feature would replay the entire apply history.
	if !hasRunTriggersFired {
		if err := db.Exec("UPDATE runs SET run_triggers_fired_at = COALESCE(completed_at, updated_at) WHERE status = 'applied' AND run_triggers_fired_at IS NULL").Error; err != nil {
			logger.Warnf("Failed to backfill runs.run_triggers_fired_at: %v", err)
		} else {
			logger.Info("Backfilled runs.run_triggers_fired_at for pre-existing applied runs (tfe_run_trigger cutover)")
		}
	}
